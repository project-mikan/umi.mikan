...
func cleanupTestData(t *testing.T, db *sql.DB) {
	// テスト名とPIDでユニークなemailプレフィックスを特定
	testPrefix := fmt.Sprintf("%s-%d-", t.Name(), os.Getpid())
	testPrefix = strings.ReplaceAll(testPrefix, "/", "-")
	testPrefix = strings.ReplaceAll(testPrefix, " ", "-")
	emailPattern := fmt.Sprintf("%%test-suite-%s%%", testPrefix)

	_, byEmailPattern, err := buildDynamicCleanupQueries(db)
	if err != nil {
		t.Logf("Warning: failed to build cleanup queries: %v", err)
		return
	}
	...
	for _, query := range byEmailPattern {
		if _, err := tx.Exec(query, emailPattern); err != nil {
