...
| 月間日記投稿数 | 20件/月 |
| プロンプトオーバーヘッド | 250トークン/回（指示文・抽出ルール）|
| 1日記あたりの平均登場人物数 | 2名 |
| 出力トークン数 | 200トークン/回（人物JSON × 2名）|
...
| 項目 | 計算 | コスト |
|---|---|---|
| 入力（日記本文 + プロンプト） | 1,500 + 250 = 1,750 tokens | $0.000131 |
| 出力（人物JSONリスト） | 200 tokens | $0.000060 |
| **1件あたり合計** | | **$0.000191** |

月20件投稿した場合:
- 月間: 0.000191 × 20 = **$0.00382/月**

### 月間合計コスト試算

| シナリオ | 月間投稿数 | 平均登場人物数 | 月額概算 |
|---|---|---|---|
| ライト（人物少なめ） | 10件 | 1名/件 | **$0.002** |
| スタンダード | 20件 | 2名/件 | **$0.004** |
| ヘビー（人物多め） | 30件 | 4名/件 | **$0.006** |

### 既存日記のバックフィルコスト（過去日記の一括処理）

過去1年分（365件）を一括処理した場合:
- 365件 × 1,750 tokens（入力） = 638,750 tokens → $0.048
- 365件 × 200 tokens（出力） = 73,000 tokens → $0.022
- **合計: $0.070 / ユーザ**（一回限り、≒ 約10円）

### 機能横断のコスト比較

| 機能 | 月額コスト（スタンダード） | 備考 |
|---|---|---|
| ADR-0009 自然言語検索 | $0.0006 | text-embedding-004が安価 |
| ADR-0010 自己分析レポート | $0.006 | 週次自動生成込み |
| ADR-0011 人間関係の可視化 | $0.004 | 毎日記ごとに抽出 |
| **3機能合計** | **$0.011** | **≒ 1.6円/月/ユーザ** |

### 総評

- 3機能を全て有効にしても月額 **約1.6円/ユーザ**（$0.011）と非常に安価
- コストの大半は自己分析レポート生成（長文テキスト）と人間関係抽出が占める
- 既存日記のバックフィルは人間関係抽出で最大10円だが一回限り
- ユーザ自身のAPIキーを使用するため、サービス側の負担はゼロ
