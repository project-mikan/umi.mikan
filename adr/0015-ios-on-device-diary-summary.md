...
Accepted（2026-07-11 追記: 要約形式を箇条書きから1文形式に変更。詳細は末尾「追記」参照）
...
## 追記（2026-07-11）: 箇条書き形式から1文形式への変更

### 問題

- `dayRow` の要約表示で、各箇条書きが `lineLimit(1)` かつ最大16文字のため、カード幅・フォントサイズの組み合わせによっては文字が見切れる（`...` で切れる）ケースが発生した
- 「重要なこと最大3点」の箇条書きは要約として簡潔だが、3つの断片的なフレーズが並ぶだけで文脈が繋がらず、かえって内容が読み取りにくいというフィードバックがあった
- さらに、20〜24文字程度の短い1文では日記に含まれる複数の要素を拾いきれず、「どんな日だったかをパッと見たい」という目的に対して情報量が不足した

### 変更内容

- 要約の形式を「箇条書き最大3点、各16文字程度」から「その日の主な出来事2〜3個を『〜して、〜で、〜だった』のように自然につないだ1文（全角40〜55文字程度、表示枠の2〜3行に収まる長さ）」に変更した
- `DiarySummaryCacheEntry.points: [String]` → `DiarySummaryCacheEntry.summary: String` に変更。あわせて要約形式バージョン（`version` フィールド + `currentVersion` 定数）を導入し、キャッシュ判定を「本文ハッシュ一致 かつ バージョン一致」にした。プロンプトや想定文字数など「本文が同じでも生成し直したい変更」をしたら `currentVersion` を上げるだけで旧形式の要約が全日再生成される（ローカルの派生キャッシュのみでサーバ・DBへの影響はないため、マイグレーション処理は行わない。旧構造のキャッシュファイルはデコード失敗により丸ごと破棄される）
- `DiarySummaryStore.parsePoints` → `DiarySummaryStore.parseSummary` に変更。改行区切りで複数行採用していたロジックをやめ、先頭の非空行のみを採用する（モデルが誤って複数行返した場合のフォールバック）
- `maxPointLength`（16）→ `maxSummaryLength`（60）に変更。60文字はターゲットの40〜55文字に対する切り詰め上限であり、`.subheadline` の3行表示にほぼ収まる長さ
- 生成プロンプトを「重要なことを最大3つ、各12〜16文字程度の体言止めのフレーズを改行区切りで出力」から「主な出来事を2〜3個拾い、自然につないだ40〜55文字程度の1文にまとめ、箇条書き・改行・記号を使わない」に変更
- `MonthlyView.dayPreview` の表示を `ForEach` による複数行の箇条書き（各 `lineLimit(1)`）から、`sparkles` アイコン + 単一の `Text`（`lineLimit(3)`、2〜3行の折り返しを許可）に変更

### 影響

- `ios/Sources/Infrastructure/DiarySummaryStore.swift`: キャッシュ構造体（バージョンフィールド追加）、キャッシュ判定、パース関数、生成プロンプトを変更
- `ios/Sources/Features/Monthly/MonthlyView.swift`: `dayPreview` の表示ロジックを変更
- `ios/Tests/DiarySummaryStoreTests.swift`: `parsePoints` のテストケースを `parseSummary` 用に置き換え
