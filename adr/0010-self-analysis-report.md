...
| 月間日記投稿数 | 20件/月 |
| プロンプトオーバーヘッド | 300トークン/回（指示文・出力フォーマット定義）|
| 出力トークン数 | 600トークン/回（構造化JSON）|
...
| 項目 | 計算 | 月間コスト |
|---|---|---|
| 入力（日記7件 + プロンプト） | (7 × 1,500 + 300) tokens × 4回 = 43,200 tokens | $0.00324 |
| 出力（分析結果JSON） | 600 tokens × 4回 = 2,400 tokens | $0.00072 |
| **週次合計** | | **$0.00396/月** |

### オンデマンド生成コスト（手動実行）

| 期間 | 入力日記数 | 入力トークン | 出力トークン | 1回あたりコスト |
|---|---|---|---|---|
| 直近7日 | 5件 | 7,800 tokens | 600 tokens | **$0.00076** |
| 直近30日 | 20件 | 30,300 tokens | 700 tokens | **$0.00248** |
| 直近90日（切り詰め）| 60件 ※ | 27,300 tokens ※ | 800 tokens | **$0.00228** |

※ 直近90日は1,500 tokens/件 × 60件 = 90,000 tokensとなりコンテキスト上限（128K）に近いため、1件あたり先頭300文字（≈ 450トークン）に切り詰めて送信

### 月間合計コスト試算

| シナリオ | 週次自動生成 | 手動生成 | 月額概算 |
|---|---|---|---|
| ライト（週次のみ） | 4回 | 0回 | **$0.004** |
| スタンダード（週次 + 月次レポート1回） | 4回 | 1回（30日） | **$0.006** |
| ヘビー（週次 + 各種レポート5回） | 4回 | 5回（混在） | **$0.016** |

### 総評

- 月額コストは **$0.004〜$0.016**（≒ 0.6〜2.4円）
- 週次自動生成を4回行っても月額約0.6円と低コスト
- 90日分の長期分析でもコンテキスト切り詰めにより1回約0.34円
- キャッシュにより同一期間の再生成は追加コストゼロ
