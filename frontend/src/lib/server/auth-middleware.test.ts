...
function createMockCookies(tokens: { accessToken?: string; refreshToken?: string } = {}) {
...
describe('auth-middleware', () => {
	...
	describe('ensureValidAccessToken', () => {
		...
		it('should refresh token when access token is missing', async () => {
			...
			expect(cookies.set).toHaveBeenCalledWith('accessToken', 'new-access-token', {
				path: '/',
				httpOnly: true,
				secure: false,
				sameSite: 'strict',
			...
			expect(cookies.set).toHaveBeenCalledWith('refreshToken', 'new-refresh-token', {
				path: '/',
				httpOnly: true,
				secure: false,
				sameSite: 'strict',
		...
		it('should refresh token when access token is expiring soon', async () => {
			...
			expect(refreshAccessToken).toHaveBeenCalledWith('refresh-token');
			expect(cookies.set).toHaveBeenCalledWith('accessToken', 'new-access-token', expect.any(Object));
		});
