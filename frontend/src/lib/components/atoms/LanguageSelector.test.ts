...
describe("LanguageSelector Component Logic", () => {
	...
	it("should handle click outside to close dropdown", () => {
		...
		// Test click inside
		const eventInside = { target: mockTargetInside } as unknown as MouseEvent;
		handleClickOutside(eventInside);
		expect(isOpen).toBe(true); // Should still be open

		// Test click outside
		const eventOutside = { target: mockTargetOutside } as unknown as MouseEvent;
	...
	it("should find current language or fallback to first option", () => {
		...
		const currentLanguage1 =
			languages.find((lang) => lang.code === currentLocale1) || languages[0];
		...
		const currentLanguage2 =
			languages.find((lang) => lang.code === currentLocale2) || languages[0];
