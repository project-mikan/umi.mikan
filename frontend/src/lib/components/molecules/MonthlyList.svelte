	...
	{#each Array.from({ length: daysInMonth }, (_, i) => i + 1) as day}
		...
		<div class="bg-white shadow rounded-lg overflow-hidden">
			<button
				...
				class="w-full p-4 text-left hover:bg-gray-50 transition-colors cursor-pointer"
			>
				<div class="flex items-center justify-between mb-2">
					...
					{#if !entry}
						<span class="text-blue-600 text-sm font-medium">+ {$_("monthly.entry")}</span>
					{/if}
				</div>
