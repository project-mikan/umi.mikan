...
- **Keyboard toolbar**: While editing diary text (Home cards and detail view), a `ToolbarItemGroup(placement: .keyboard)` shows 保存 and a dismiss-keyboard button above the keyboard. Home tracks the focused card with `@FocusState` (`DiaryCardFocus`) so 保存 saves the card being edited.
