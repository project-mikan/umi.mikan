...
**CI注意事項**: iOS テストは GitHub Actions（`.github/workflows/301_ios_test.yml`）で `CODE_SIGNING_ALLOWED=NO` の環境で実行される。`Keychain` を使うコードは `SecItem` 系 API が失敗する場合を考慮する必要がある（entitlements 未設定のため）。`KeychainStore` は Keychain が使えない環境では `UserDefaults` にフォールバックする実装になっている（`ios/Sources/Infrastructure/KeychainStore.swift`）。
