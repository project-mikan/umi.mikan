...
- **Swipe navigation in the half-modal**: `DiaryDetailSheet` takes `items` + `initialIndex`; horizontal swipes move to the previous/next diary. Ordering per caller: Search = the current result list order, Home = today/yesterday/day-before-yesterday, Monthly = every day of the displayed month.
